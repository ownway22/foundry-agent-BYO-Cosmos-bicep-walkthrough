...
UMI_PRINCIPAL_ID="$(json_value '.identity.userAssignedIdentities | to_entries[0].value.principalId' <<<"${project_json}")"
...
[[ -n "${UMI_PRINCIPAL_ID}" ]] || fail "Project UMI principal ID is empty"
[[ -n "${PROJECT_WORKSPACE_ID}" ]] || fail "Project workspace ID is empty"
pass "Project has system-assigned and user-assigned identities"

AZUREML_CONTAINER_ID="${STORAGE_ACCOUNT_ID}/blobServices/default/containers/${PROJECT_WORKSPACE_ID}-azureml-blobstore"
AGENTS_CONTAINER_ID="${STORAGE_ACCOUNT_ID}/blobServices/default/containers/${PROJECT_WORKSPACE_ID}-agents-blobstore"
...
az role assignment list --assignee "${PROJECT_PRINCIPAL_ID}" --scope "${AZUREML_CONTAINER_ID}" --role "Storage Blob Data Contributor" --query '[0].id' -o tsv | grep -q . || fail "Storage Blob Data Contributor assignment missing for project SMI"
az role assignment list --assignee "${UMI_PRINCIPAL_ID}" --scope "${AZUREML_CONTAINER_ID}" --role "Storage Blob Data Contributor" --query '[0].id' -o tsv | grep -q . || fail "Storage Blob Data Contributor assignment missing for project UMI"
az role assignment list --assignee "${PROJECT_PRINCIPAL_ID}" --scope "${AGENTS_CONTAINER_ID}" --role "Storage Blob Data Owner" --query '[0].id' -o tsv | grep -q . || fail "Storage Blob Data Owner assignment missing for project SMI"
az role assignment list --assignee "${UMI_PRINCIPAL_ID}" --scope "${AGENTS_CONTAINER_ID}" --role "Storage Blob Data Owner" --query '[0].id' -o tsv | grep -q . || fail "Storage Blob Data Owner assignment missing for project UMI"
...
az role assignment list --assignee "${UMI_PRINCIPAL_ID}" --scope "${SEARCH_SERVICE_ID}" --role "Search Index Data Contributor" --query '[0].id' -o tsv | grep -q . || fail "Search Index Data Contributor assignment missing for project UMI"
az role assignment list --assignee "${UMI_PRINCIPAL_ID}" --scope "${SEARCH_SERVICE_ID}" --role "Search Service Contributor" --query '[0].id' -o tsv | grep -q . || fail "Search Service Contributor assignment missing for project UMI"
